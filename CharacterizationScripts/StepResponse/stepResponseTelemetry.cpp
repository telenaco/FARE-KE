...
#define STEP_MAX_PWM 255 // Max PWM value for the step response
#define STEP_MIN_PWM 0   // Min PWM value for the step response
#define STEP_DURATION 2000 // Duration in milliseconds for each step state
...
unsigned long lastStepChange = 0; // Timestamp of the last step change
bool isStepHigh = false; // Current state of the step
int lastPWMValue = -1; // Track the last PWM value to avoid redundant updates
...
void stepController() {
    ...
    // Check if it's time to toggle the step state
    if (currentMillis - lastStepChange > STEP_DURATION) {
        // Toggle the step state
        isStepHigh = !isStepHigh;
        lastStepChange = currentMillis;

        // Determine the new PWM value based on the step state
        int newPWMValue = isStepHigh ? STEP_MAX_PWM : STEP_MIN_PWM;

        // Update the actuator if the PWM value has changed
        if (newPWMValue != lastPWMValue) {
            // Update actuator via PWM
            analogWrite(MOTOR_PWM_PIN, newPWMValue);

            // If using a Serial-based actuator, send the command via Serial1
            Serial1.println(newPWMValue);
            ...
            lastPWMValue = newPWMValue; // Remember the last PWM value sent
        }
    }
}
...
void setup() {
  ...
  -0.0173116,  0.00683699, 0.106079,   0.0170052,   -0.00432406,  1.00594 };
    ...
    // Prepare for step response
    pinMode(MOTOR_PWM_PIN, OUTPUT);
}

void loop() {
    // Perform step response control
    stepController(); // This function will toggle the step state and update the actuator
    ...
    if (mcpScale->updatedAdcReadings()) {
        ...
        output.concatf("%d", lastPWMValue); // Add current PWM value, which is managed by stepController()
