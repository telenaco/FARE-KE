...
class HX711 {
 public:
  ...
  // enumeration to select the conversion mode
  enum ConversionMode { SINGLE_VALUE, LINEAR, POLYNOMIAL };
  ...
  // methods to set the linear and polynomial calibration coefficients
 ...
 private:
  ...
  // variables for linear calibration
  float _linearSlope = 1.0;
  float _linearIntercept = 0.0;

  // variables for polynomial calibration
  float _polyA = 0.0;
  float _polyB = 1.0;
  float _polyC = 0.0;

  ConversionMode _conversionMode = SINGLE_VALUE;
};
