...
// `interrupts()` calls.
  ...
  float HX711::get_units(byte times) {
    ...
    switch (_conversionMode) {
      ...
      case LINEAR:
        return _linearSlope * value + _linearIntercept;

      case POLYNOMIAL:
        return _polyA * value * value + _polyB * value + _polyC;
  ...
  void HX711::tare(byte times) {
    long avgReading = read_average(times);

    switch (_conversionMode) {
      case SINGLE_VALUE:
        // For single value, just set the reading offset.
        set_offset(avgReading);
        break;

      case LINEAR:
        // For linear, adjust the intercept based on the average reading and the
        // pre-determined slope.
        _linearIntercept = -_linearSlope * avgReading;
        break;

      case POLYNOMIAL:
        // For polynomial, adjust the constant term (C) such that the equation
        // becomes: y = A * x^2 + B * x + (C - avgReading).
  ...
  void HX711::setLinearCalibration(float slope, float intercept) {
    _linearSlope = slope;
    _linearIntercept = intercept;
  }

  void HX711::setPolynomialCalibration(float a, float b, float c) {
    _polyA = a;
    _polyB = b;
    _polyC = c;
  }

  void HX711::setConversionMode(HX711::ConversionMode mode) {
