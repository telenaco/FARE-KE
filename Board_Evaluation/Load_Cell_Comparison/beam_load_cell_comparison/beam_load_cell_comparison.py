"""
Filename: beam_load_cell_comparison.py

Description:
    Analysis module for comparing performance characteristics of different beam-type load cells.
    Processes calibration data, calculates linearity metrics, and performs comparative analysis
    of load cell behavior under different zero adjustment methods.
    Visualizes performance metrics including accuracy, drift, and noise characteristics.
...
def linear_analysis(data_df, title_suffix):
    """
    Perform linear regression analysis on load cell data and visualize results.
    
    Parameters:
        data_df (pd.DataFrame): DataFrame containing the load cell data.
...
def calculate_accuracy(data_df, title_suffix):
    """
    Calculate accuracy metrics for a load cell based on linear model residuals.
    
    Parameters:
        data_df (pd.DataFrame): DataFrame containing the load cell data.
        title_suffix (str): Suffix identifying the analysis condition.
    """
    model = LinearRegression()
...
def combine_images1(titles, output_filename):
    """
    Combine multiple images by their titles into a single image.
    
    Parameters:
        titles (list): List of image titles to combine.
        output_filename (str): Filename for the combined image.
    """
    # Open the images
...
def calibration_analysis(data_df, reading_column, filename):
    ...
    Parameters:
        data_df (pd.DataFrame): DataFrame containing the load cell data.
        reading_column (str): Column name for the load cell readings.
        filename (str): Filename for saving the output plot.
        
    Returns:
        dict: Dictionary containing calibration results and metrics.
    """
    data = data_df.copy()    
...
def combine_images(filenames, output_filename):
    ...
    Parameters:
        filenames (list): List of image filenames to combine.
        output_filename (str): Filename for the combined image.
    """
    # Open the images
...
for index, calibration_cell in enumerate(load_cells):
    ...
    readings_grams = [convert_to_grams(noise_cell, val) for val in readings]
    
    # Center the readings gf around zero by subtracting the mean
    centered_readings_grams = readings_grams - np.mean(readings_grams)
...
def sanitize_filename(filename):
    ...
    Returns:
        str: Sanitized filename with only alphanumeric characters.
    """
...
def compute_sampling_rate(micros):
    """
    Compute sampling rate from microsecond timestamps.
    
    Parameters:
        micros (np.array): Array of microsecond timestamps.
        
    Returns:
        float: Computed sampling rate in Hz.
    """
...
def middle_avg(data):
    ...
    Returns:
        float: Average of the middle 90% of data.
    """
...
def middle_ten_percent(data):
    """
    Get the middle ten percent of the data.
    
    Parameters:
        data (array-like): Input data array.
        
    Returns:
        array-like: Middle 10% subset of the input data.
    """
...
def extend_to_max_length(data, max_length):
    """
    Extend data to a specified maximum length by repeating middle values.
    
    Parameters:
        data (array-like): Input data array.
        max_length (int): Target length for the data.
        
    Returns:
        array-like: Extended data array.
    """
    iterations = 0
...
def simple_identify_intervals(load_cell_data, timestamps):
    """
    Identify loading and unloading intervals in the load cell data.
    
    Parameters:
        load_cell_data (array-like): Load cell readings.
        timestamps (array-like): Corresponding timestamps.
        
    Returns:
        tuple: Two lists of tuples containing (start_idx, end_idx) for loading and unloading intervals.
    """
...
def adjust_intervals(loading_intervals, unloading_intervals, data_length):
    ...
    Parameters:
        loading_intervals (list): List of (start, end) tuples for loading intervals.
        unloading_intervals (list): List of (start, end) tuples for unloading intervals.
        data_length (int): Total length of the data.
        
    Returns:
        tuple: Two lists of adjusted (start, end) tuples for loading and unloading intervals.
    """
...
def standardize_load_cell_data_debug_updated(load_cell_data, loading_intervals, unloading_intervals, timestamps):
    """
    Standardize the loading and unloading events for a given load cell data.
    ...
    Returns:
        tuple: Two arrays containing standardized data and timestamps.
    """
...
def process_and_plot_load_cell(load_cell_name, color, subplot_position):
    """
    Process and plot load cell data with identified transitions.
    
    Parameters:
        load_cell_name (str): Name of the load cell.
        color (str): Color to use for plotting.
        subplot_position (int): Position in the subplot grid.
        
    Returns:
        tuple: Conditions, transitions, and processed data.
    """
...
def compute_average_for_conditions(conditions, transitions, data):
    """
    Compute average values for each condition between transitions.
    
    Parameters:
        conditions (list): List of condition names.
        transitions (list): List of transition indices.
        data (array-like): Data values.
        
    Returns:
        dict: Dictionary mapping conditions to lists of average values.
    """
    averages = {}
...
def plot_with_identified_transitions_corrected(load_cell_name, color, subplot_position, data, timestamps):
    """
    Plot load cell data with identified transitions between loading and unloading states.
    
    Parameters:
        load_cell_name (str): Name of the load cell.
        color (str): Color to use for plotting.
        subplot_position (int): Position in the subplot grid.
        data (array-like): Load cell readings.
        timestamps (array-like): Corresponding timestamps.
        
    Returns:
        tuple: Lists of conditions and transitions.
    """
...
def compute_descriptive_stats(load_test_data, load_cell, loading_intervals_map, unloading_intervals_map):
    ...
    Parameters:
        load_test_data (pd.DataFrame): DataFrame containing load test data.
        load_cell (str): Name of the load cell.
        loading_intervals_map (dict): Dictionary mapping load cells to loading intervals.
        unloading_intervals_map (dict): Dictionary mapping load cells to unloading intervals.
        
    Returns:
        dict: Dictionary of statistical measures for each loading/unloading event.
    """
    stats = {}
...
def compute_repeatability(load_test_data, load_cell, intervals):
    ...
    Parameters:
        load_test_data (pd.DataFrame): DataFrame containing load test data.
        load_cell (str): Name of the load cell.
        intervals (list): List of (start, end) tuples defining intervals.
        
    Returns:
        list: List of deviations from the initial mean.
    """
    deviations = []
