"""
Filename: mcp356x_three_axis_calibration.py

Description:
    Calibration module for three-axis load cells using the MCP356x ADC system.
    Processes calibration data from CSV files to determine optimal calibration 
    transformations between ADC readings and applied forces.
    Implements both matrix-based (least squares) and polynomial regression 
    calibration methods, with detailed error analysis and validation.
...
# Append the estimated forces and corresponding error matrices to the log messages
...
log_file_name = f"{csv_base_name}_log.md"  # This adds "_log.md" to the base file name
...
# Load the validation data needs readings different than those used for calibration
